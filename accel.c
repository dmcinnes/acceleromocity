...
void readRegisters(byte addressToRead, int bytesToRead, byte * dest)
{
  ...
  for(int x = 0 ; x < bytesToRead-1 ; x++)
    dest[x] = i2c_read(1);                    // read one byte and request more

  dest[bytesToRead-1] = i2c_read(0);                    // read one byte and stop
}
